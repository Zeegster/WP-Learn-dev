...
function updateFlexBasis() {
	...
	columnBlocks.forEach((block) => {
		...
		classList.forEach((className) => {
			const mobileMatch = className.match(/is-column-width-mobile-(\S+)/);
			const tabletMatch = className.match(/is-column-width-tablet-(\S+)/);
	...
	for (let i = 0; i < swiperEl.length; i++) {
		...
		for (let j = 0; j < attributes.length; j++) {
			...
			if (attribute.name == "data-carousel-settings") {
				...
				swiperEl[i].setAttribute("data-carousel-settings", newCarouselSettings);
